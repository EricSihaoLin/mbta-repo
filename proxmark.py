...
device = usb.core.find(idVendor=PROXMARK3_USB_VENDOR_MAGIC, idProduct=PROXMARK3_USB_PRODUCT_MAGIC, backend=libusb1_backend)
...
class UsbCommandReceiver(threading.Thread):
    ...
    def run(self):
        ...
        while(not self.stopping):
            try:
                ret = device.read(PROXMARK3_USB_ENDPOINT_IN, PROXMARK3_USB_INTERFACE, 500)
...
commandReceiver.subscribe(proxmarkcmd.CMD_DEBUG_PRINT_STRING, handleDebugPrint)
commandReceiver.subscribe(proxmarkcmd.CMD_MEASURED_ANTENNA_TUNING, handleAntennaTuning)
